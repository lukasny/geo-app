...
function sanitizeBlogBody(html: string): string {
  ...
  // Also touch the audit-engine sanitizer to catch any em-dashes Claude
  // slipped in despite the prompt instruction. Re-using its conservative
  // tag handling on the already-sanitized body is a no-op for tags but
  // a safety net for the no-em-dash rule we've layered into prompts.
...
/** Count blog posts generated by this store since the start of the current
 *  calendar month. Used to enforce maxBlogPostsPerMonth. Includes deleted
 *  drafts (Claude API was already paid, so it still counts) but excludes
 *  posts that were never persisted (errored mid-generation). */
